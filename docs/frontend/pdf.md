...
| 沉浸 | 底部换页栏旁切换；全屏 + 限宽居中 |
| 位置 | 记忆阅读位置 |
| 文中链接 | Link annotation 覆盖层：citation / 图表·公式交叉引用 / 章节 GoTo 点击跳页，URI 开系统浏览器。**Citation hover**：hyperref `cite.<bibtexKey>` → sidecar `rawKey`；因 PDFium 只返回解析后的 `pageIndex + y`，打开论文后（主线程空闲时）建 `pageIndex:pdfY → bibtexKey` 索引（`lib/pdf/citation-dest-keys.ts`）。**Crossref hover**（`Fig. 3` / `Table 1` / `Eq. (2)`）：同样建 dest 坐标 → kind 索引，叠加 layout region 裁剪预览；ACS 等 `/FitR` 整页目标会让同页多个 float 共用一个坐标，此时改走 **Link annotation rect → dest 名**（`mk:tbl1` / `mk:fig3`）消歧，不再依赖脆弱的划词文本。pdf-lib 无懒解析，解析跑在实体文件 Worker（`citation-dest-keys.worker.ts`）；字节优先复用 `tab.pdfBytes`，结果按 `pdfPath:size` 缓存。索引 / layout 未就绪时不弹卡片；无法消歧的链接不显示卡片。章节等非 float 内部链接只保留导航 |
