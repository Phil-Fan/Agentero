...
| 区域 | 行为 |
|---|---|
| 左栏 | 「全部」+ 订阅列表；选中过滤右栏。行：标题、失败时一行错误。右键：重命名 / 删除 / 复制 URL / 立即刷新 |
| 右栏 | 时间倒序。`@tanstack/react-virtual` 窗口化。按日分组（「今天」「昨天」或日期）可选，MVP 不做未读徽章 |
| 顶条 | 过滤芯片「全部 / 论文 / 其它」；手动刷新（当前源或全部） |
| 空态（无订阅） | 一句说明 + 添加框；三行提示：任意 feed URL；arXiv 芯片；X / Scholar 需先有 RSS |
| 空态（有订阅无条目） | 「还没有条目，点刷新」或显示该源 `lastError` |
| 分栏 | `ResizablePanel`；左栏默认约 220px，可拖 |
