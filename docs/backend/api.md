...
```ts
{
  id?: string; // 省略则新建
  name: string;
  template?: 'opencode' | 'openclaw' | 'gemini' | 'hermes' | 'claude-acp' | 'codex-acp' | 'qodercli' | 'grok-build' | 'pi' | 'dsh' | 'custom';
  command: string;
  args?: string[];
